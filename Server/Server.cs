...
namespace Server
{
    class Server
    {
        ...
        /// </summary>
        /// <param name="p"></param>
        public static void DataManager(Packet p,Socket s)
        {
            switch (p.packetType)
            {
                ...
                case PacketType.CloseConnection:
                    var exitClient = GetClientByID(p,s);
        ...
        /// <param name="p"></param>
        /// <returns></returns>
        private static ClientData GetClientByID(Packet p,Socket s)
        {
            return (from client in _clients
                    where client.clientSocket.RemoteEndPoint == s.RemoteEndPoint
                    select client)
                    .FirstOrDefault();
